...
def ventana_about(window):
    ...
    about_text = Label(window, text="Información sobre autores", font=("Courier", 24), fg='white', bg="#9467fe")
...
def estadisticas(window):
    ...
    stats_text = Label(window, text="Estadísticas del Juego", font=("Courier", 24), fg='white', bg="#9467fe")
    stats_text.pack(pady=20)
    
    # Example statistics
    stats_info = """ 
    Total Juegos Jugados: 10
    Total Puntos: 5000
    Promedio de Puntos por Juego: 500
    Máximo Puntaje en un Juego: 1000
    """
    stats_label = Label(window, text=stats_info, font=("Courier", 16), fg='white', bg="#9467fe", wraplength=500, justify=LEFT)
...
def mostrar_estadisticas(window, player_name, score, elapsed_time):
    window.destroy()
    window = Tk()
    window.configure(bg="#faf5e8", cursor='hand2')
    window.geometry("600x800")
    window.title("Estadísticas del Jugador")
    ...
    stats_text = Label(window, text="Estadísticas del Jugador", font=("Courier", 24), fg='white', bg="#9467fe")
    stats_text.pack(pady=20)
    
    stats_info = f""" 
    Nombre del Jugador: {player_name}
    Total Puntos: {score}
    Tiempo de Juego: {elapsed_time // 60:02}:{elapsed_time % 60:02}
    """
    stats_label = Label(window, text=stats_info, font=("Courier", 16), fg='white', bg="#9467fe", wraplength=500, justify=LEFT)
    stats_label.pack(pady=10)
...
def mostrar_estadisticas_multiplayer(window, player_names, scores, elapsed_time):
    window.destroy()
    window = Tk()
    window.configure(bg="#faf5e8", cursor='hand2')
    window.geometry("600x800")
    window.title("Estadísticas del Juego Multijugador")
    ...
    stats_text = Label(window, text="Estadísticas del Juego Multijugador", font=("Courier", 24), fg='white', bg="#9467fe")
    stats_text.pack(pady=20)

    stats_info = ""
    for i, player_name in enumerate(player_names):
        stats_info += f""" 
        Nombre del Jugador {i+1}: {player_name}
        Total Puntos: {scores[i]}
        """
    stats_info += f"""
    Tiempo de Juego: {elapsed_time // 60:02}:{elapsed_time % 60:02}
    """
...
def game_single_player(window, player_name, image_path):
    ...
    def end_game():
        ...
        game_ended = True
        elapsed_time = int(time.time() - start_time)
        mostrar_estadisticas(window, player_name, score, elapsed_time)
        # Show statistics automatically when the game ends
        mostrar_estadisticas(window, player_name, score, int(time.time() - start_time))
    window.mainloop()
...
def multiplayer(window):
    ...
    def check_start_game(*args):
        if var1.get() + var2.get() + var3.get() == 2:
            player_names = []
            if var1.get() == 1:
                player_names.append(entry_jug1.get())
            if var2.get() == 1:
                player_names.append(entry_jug2.get())
            if var3.get() == 1:
                player_names.append(entry_jug3.get())
            game_multiplayer(window, player_names)
...
def game_multiplayer(window, player_names):
    ...
    def lose_shot():
        ...
        if shots_remaining[current_player] > 0:
            shots_remaining[current_player] -= 1
            shots_labels[current_player].config(text=f"Shots: {shots_remaining[current_player]}")
            if shots_remaining[current_player] == 0:
                if current_player == 0:
                    current_player = 1
                    shots_remaining[current_player] = 3
                    shots_labels[current_player].config(text=f"Shots: {shots_remaining[current_player]}")
                else:
                    end_game()

    # Function to check if the ball has not scored in 15 seconds for the current player
    def check_no_score():
        if not game_ended:
            lose_shot()
            window.after(15000, check_no_score)
    ...
    # Function to end the game and show statistics for both players
    def end_game():
        nonlocal game_ended
        game_ended = True
        elapsed_time = int(time.time() - start_time)
        mostrar_estadisticas_multiplayer(window, player_names, scores, elapsed_time)
        # Show statistics automatically when the game ends
        mostrar_estadisticas_multiplayer(window, player_names, scores, int(time.time() - start_time))
#############################################################################################
def profile2(window):
    window.destroy()
    window = Tk()
...
def game_single_player2(window, player_name, image_path):
    # Destroy the previous window and create a new one
    window.destroy()
    window = Tk()
    window.configure(bg="pink", cursor='hand2')
